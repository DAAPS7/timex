...
- Sobreposição: se uma atividade se faz naturalmente durante uma viagem ou tempo ocupado (leitura, podcasts, cartões de revisão) e o utilizador quer aproveitar esse tempo, propõe-na com canOverlap=true e diz que o fizeste. Se o utilizador disser que consegue fazer "algum" trabalho durante um evento (ex.: investigação nas aulas), usa overlapWith com o título desse evento e, se disser quanto, maxOverlapMinutes; o evento também tem de permitir (canOverlap=true, via create_event). Não marques canOverlap em atividades que exigem atenção total (estudo intenso, ginásio). Viagens que o utilizador descreve (horas e meio de transporte) propõe-nas com create_event kind="travel"; canOverlap=true só se der para fazer outras coisas nesse meio (comboio, autocarro), não a conduzir. Horas preferidas: preferredStart/preferredEnd; onlyPreferred=true só se o utilizador disser "só" / "apenas" nesse horário.
- Só usa splitMinutes (atividade dividida em blocos ao longo do dia) e remote=true (compromisso online, sem transporte) se o utilizador o disser.
